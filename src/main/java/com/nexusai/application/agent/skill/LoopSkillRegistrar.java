...
public class LoopSkillRegistrar {
    ...
    public static final String LOOP_PROMPT_TEMPLATE = """
        ...
        |-----------------------|---------------------|------------------------------------------|
        | `Nm` where N ≤ 59   | `0 */N * ? * *`   | every N minutes                          |
        | `Nm` where N ≥ 60   | `0 0 */H ? * *`   | round to hours (H = N/60, must divide 24)|
        | `Nh` where N ≤ 23   | `0 0 */N ? * *`   | every N hours                            |
        | `Nd`                | `0 0 0 */N * ?`   | every N days at midnight local           |
        | `Ns`                | treat as `ceil(N/60)m` | cron minimum granularity is 1 minute  |

        All expressions above are 6-field (seconds first). Exactly one of day-of-month / day-of-week must be `?` — the unused one is always `?`, never `*`.

        **If the interval doesn't cleanly divide its unit** (e.g. `7m` → `0 */7 * ? * *` gives uneven gaps at :56→:00; `90m` → 1.5h which cron can't express), pick the nearest clean interval and tell the user what you rounded to before scheduling.
