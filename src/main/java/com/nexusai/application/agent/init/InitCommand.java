package com.nexusai.application.agent.init;
...
/**
 * /init command · 对齐 CC commands/init.ts.
 *
 * <p>CC source: commands/init.ts (256 LOC). Two prompts:
 * - OLD_INIT_PROMPT (legacy CLAUDE.md generator)
 * - NEW_INIT_PROMPT (multi-phase: ask what to set up, explore codebase, fill gaps, write CLAUDE.md, write CLAUDE.local.md, suggest skills, suggest optimizations)
 *
 * Dispatches based on feature('NEW_INIT') + USER_TYPE=ant || CLAUDE_CODE_NEW_INIT env.
 */
public final class InitCommand {
    ...
    public static final String PROGRESS_MESSAGE = "analyzing your codebase";
    public static final String SOURCE = "builtin";

    public static final String OLD_PROMPT = """
    ...
    public static final String NEW_PROMPT = """
        ...
        Launch a subagent to survey the codebase, and ask it to read key files to understand the project: manifest files (package.json, Cargo.toml, pyproject.toml, go.mod, pom.xml, etc.), README, Makefile/build configs, CI config, existing CLAUDE.md, .nexusai/rules/, AGENTS.md, .cursor/rules or .cursorrules, .github/copilot-instructions.md, .windsurfrules, .clinerules, .mcp.json.
        ...
        - Existing .nexusai/skills/ and .nexusai/rules/ directories
        ...
        If the user chose personal CLAUDE.local.md or both: ask about them, not the codebase. Do not mark any options as "recommended" — this is about their personal preferences, not best practices. Examples of questions:
          ...
          - Only if Phase 2 found multiple git worktrees: ask whether their worktrees are nested inside the main repo (e.g., `.nexusai/worktrees/<name>/`) or siblings/external (e.g., `../myrepo-feature/`). If nested, the upward file walk finds the main repo's CLAUDE.local.md automatically — no special handling needed. If sibling/external, the personal content should live in a home-directory file (e.g., `~/.nexusai/<project-name>-instructions.md`) and each worktree gets a one-line CLAUDE.local.md stub that imports it: `@~/.nexusai/<project-name>-instructions.md`. Never put this import in the project CLAUDE.md — that would check a personal reference into the team-shared file.
        ...
        **Consume `note` entries from the Phase 3 preference queue whose target is CLAUDE.md** (team-level notes) — add each as a concise line in the most relevant section. These are the behaviors the user wants Claude to follow but didn't need guaranteed (e.g., "propose a plan before implementing", "explain the tradeoffs when refactoring"). Leave personal-targeted notes for Phase 5.
        ...
        For projects with multiple concerns, suggest organizing instructions into `.nexusai/rules/` as separate focused files (e.g., `code-style.md`, `testing.md`, `security.md`). These are loaded automatically alongside CLAUDE.md and can be scoped to specific file paths using `paths` frontmatter.
        ...
        **Consume `note` entries from the Phase 3 preference queue whose target is CLAUDE.local.md** (personal-level notes) — add each as a concise line. If the user chose personal-only in Phase 1, this is the sole consumer of note entries.
        ...
        If Phase 2 found multiple git worktrees and the user confirmed they use sibling/external worktrees (not nested inside the main repo): the upward file walk won't find a single CLAUDE.local.md from all worktrees. Write the actual personal content to `~/.nexusai/<project-name>-instructions.md` and make CLAUDE.local.md a one-line stub that imports it: `@~/.nexusai/<project-name>-instructions.md`. The user can copy this one-line stub to each sibling worktree. Never put this import in the project CLAUDE.md. If worktrees are nested inside the main repo (e.g., `.nexusai/worktrees/`), no special handling is needed — the main repo's CLAUDE.local.md is found automatically.
        ...
        **Then suggest additional skills** beyond the queue when you find:
        ...
        If `.nexusai/skills/` already exists with skills, review them first. Do not overwrite existing skills — only propose new ones that complement what is already there.

        Create each skill at `.nexusai/skills/<skill-name>/SKILL.md`:
        ...
        - **Proposal-sourced hooks** (if user chose "Skills + hooks" or "Hooks only"): Consume `hook` entries from the Phase 3 preference queue. If Phase 2 found a formatter and the queue has no formatting hook, offer format-on-edit as a fallback. If the user chose "Neither" or "Skills only" in Phase 1, skip this bullet entirely.
          ...
          1. Target file: default based on the Phase 1 CLAUDE.md choice — project → `.nexusai/settings.json` (team-shared, committed); personal → `.nexusai/settings.local.json`. Only ask if the user chose "both" in Phase 1 or the preference is ambiguous. Ask once for all hooks, not per-hook.
          ...
          3. **Load the hook reference** (once per `/init` run, before the first hook): invoke the Skill tool with `skill: 'update-config'` and args starting with `[hooks-only]` followed by a one-line summary of what you're building — e.g., `[hooks-only] Constructing a PostToolUse/Write|Edit format hook for .nexusai/settings.json using ruff`. This loads the hooks schema and verification flow into context. Subsequent hooks reuse it — don't re-invoke.
    ...
    private final BooleanSupplier newInitFeatureSupplier;
    private final Supplier<String> userTypeSupplier;
    private final BooleanSupplier newInitEnvSupplier;
    private final Runnable maybeMarkComplete;

    public InitCommand(BooleanSupplier newInitFeatureSupplier,
                        Supplier<String> userTypeSupplier,
                        BooleanSupplier newInitEnvSupplier,
                        Runnable maybeMarkComplete) {
        this.newInitFeatureSupplier = newInitFeatureSupplier;
        this.userTypeSupplier = userTypeSupplier;
        this.newInitEnvSupplier = newInitEnvSupplier;
        this.maybeMarkComplete = maybeMarkComplete;
    }

    public String name() { return NAME; }
    public String progressMessage() { return PROGRESS_MESSAGE; }
    public String source() { return SOURCE; }
    public String description() {
        if (newInitFeatureSupplier.getAsBoolean()) {
            String userType = userTypeSupplier.get();
    ...
    public java.util.List<Map<String, Object>> getPromptBlocks() {
        if (maybeMarkComplete != null) maybeMarkComplete.run();
        return List.of(Map.of("type", "text", "text", getPrompt()));
    }
}
