...
/**
 * [P0-2 / MCP-SEC-01] AgentMcpTool.checkPermissions → Passthrough · 对齐 CC
 ...
 * 1a/1b 层先于 1c 命中。子 agent 全局 MCP 工具 {@link AgentMcpTool} 若沿用 {@code Tool}
 * 接口默认 {@code Allow}（Tool.java:287-295），1c 层 {@code CheckLayer1c_ToolCheck} 直接
 * 短路放行 → mcp__ 工具在「无规则 + 非 bypass」下被<b>静默执行</b>（CC 会弹窗 ask），
 * 且 auto-mode 分类器永不覆盖（放权升级）。主 agent 版 McpServerTool（MCP-SEC-02）存在同源
 * 缺陷，本类与其孪生（同分支同 commit 落地，MCP-SEC-01 闭环）。
 *
 * <p>本测试锁 {@code checkPermissions} 必须返 {@link PermissionResult.Passthrough}（而非
 * 默认 Allow），让管线继续到 1d/1e/1f/1g → 2a/2b → 第 3 层转 ask。
 */
@DisplayName("[MCP-SEC-01] AgentMcpTool.checkPermissions 返 Passthrough（对齐 CC MCPTool.ts:56-61）")
class AgentMcpToolCheckPermissionsTest {

    private static final ObjectMapper MAPPER = new ObjectMapper();

    private static AgentMcpTool tool() {
        return new AgentMcpTool("filesystem", "read_file", "mcp__filesystem__read_file",
    ...
    /** 权限表态阶段不触碰 transport 的无操作实现（checkPermissions 无 transport 交互）。 */
    private static final class NoopTransport implements McpTransport {
        @Override public void start(TransportConfig config) {}
        @Override public CompletableFuture<com.fasterxml.jackson.databind.JsonNode> sendRequest(
