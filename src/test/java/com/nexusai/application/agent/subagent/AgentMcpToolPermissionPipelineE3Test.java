...
/**
 * [MCP-SEC-01] E3 集成测试：子 agent mcp__ 工具（{@link AgentMcpTool}）权限决策三场景
 * （真实 {@link AgentMcpTool} + 真实 {@link PermissionPipeline} 10 层管线）。
 *
 * <p><b>WHY（CLAUDE.md 规则九 · 测试验证意图）</b>: 单元测试
 * {@link AgentMcpToolCheckPermissionsTest} 只锁「checkPermissions 返 Passthrough」，
 * 未证明「管线级决策」——即 Passthrough 经 1c → 1d..2b → 第 3 层转 Ask（而非 Allow）。
 * 本测试锁三个管线级场景，编码子 agent mcp__ 路径的安全底线：
 * <ol>
 *   <li><b>无规则 + DEFAULT → Ask</b>：mcp__ 工具不得被 1c 默认 Allow 短路静默放行
 *       （CC permissions.ts:1299-1318 第 3 层 passthrough→ask 兜底）。修复前
 *       {@code AgentMcpTool} 继承 {@code Tool.java:287-295} 默认 Allow → 1c 短路 →
 *       管线返 Allow，本测试会 FAIL（RED）。</li>
 *   <li><b>deny 规则 mcp__server / mcp__server__tool → Deny</b>：回归 1a 前置——deny
 *       规则（含 MCP whole-server / whole-tool 前缀匹配 {@link
 *       com.nexusai.application.agent.permission.check.RuleQuery#toolMatchesRule}）
 ...
 * </ol>
 *
 * <p>包选择：本测试置于 {@code com.nexusai.application.agent.subagent}（非 permission 包），
 * 以便直接构造 package-private 的真实 {@link AgentMcpTool}（MCP-SEC-01 修复落点），
 * 避免用桩工具替代真实 mcp__ 工具（桩无法复现修复前「默认 Allow → 1c 短路」的 RED 行为）。
 * auto-mode 分类器依赖经反射注入（与主 agent 版 McpServerToolPermissionPipelineE3Test 的
 * {@code inject} 模式一致，字段为 package-private {@code @Autowired(required=false)} 无 setter）。
 */
@DisplayName("[MCP-SEC-01] 子 agent mcp__ 工具权限决策三场景（真实 AgentMcpTool + PermissionPipeline）")
class AgentMcpToolPermissionPipelineE3Test {
    ...
    /** 真实 AgentMcpTool（MCP-SEC-01 checkPermissions → Passthrough 落点）。 */
    private static AgentMcpTool realMcpTool() {
        return new AgentMcpTool(SERVER, TOOL, MCP_FULL_NAME,
            MAPPER.createObjectNode(), null, null, "read a file",
    ...
    void noRule_defaultMode_returnsAsk_notAllow() {
        ...
        //   修复前 AgentMcpTool 继承 Tool 默认 Allow → 1c 直接短路 → 管线返 Allow，
    ...
    void defaultAllowMcpStub_returnsAllow_documentsRedMechanism() {
        ...
        //   （Tool.java:287-295），管线即返 Allow」——即修复前 AgentMcpTool 的行为，
    ...
    // ────────────────────────────────────────────────────────────────────
    // 辅助：auto-mode 依赖注入 + 桩工具 + fake 分类器 + noop transport
    // ────────────────────────────────────────────────────────────────────
    ...
    /** 权限决策阶段不触碰 transport 的无操作实现（checkPermissions/pipeline 无 transport 交互）。 */
