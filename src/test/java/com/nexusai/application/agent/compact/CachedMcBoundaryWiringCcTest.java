 ...
 *       {@link MicroCompactor#maybeCreateMicrocompactBoundaryMessage(long, String)}（本任务生产接线）。</li>
...
class CachedMcBoundaryWiringCcTest {
    ...
    /** 本文件统一会话键 · 批 3c：会话 = 显式 sessionId（流结束用例另用 AgentState 会话）。 */
    private static final String SESSION = "s2";
    ...
    void boundary_featureOff_noConsume_noYield() {
        MicroCompactor.setCachedMicrocompactEnabled(false);
        MicroCompactor.setPendingCacheEditsForTest(edits("auto", List.of("t1"), 0L), SESSION);

        assertThat(MicroCompactor.maybeCreateMicrocompactBoundaryMessage(100L, SESSION))
    ...
    void boundary_noPendingEdits_noYield() {
        ...
        assertThat(MicroCompactor.maybeCreateMicrocompactBoundaryMessage(100L, SESSION)).isNull();
        assertThat(MicroCompactor.consumePendingCacheEdits(SESSION)).isNull();
    }
    ...
    void boundary_deltaZero_noYield() {
        MicroCompactor.setCachedMicrocompactEnabled(true);
        MicroCompactor.setPendingCacheEditsForTest(edits("auto", List.of("t1"), 50L), SESSION);

        assertThat(MicroCompactor.maybeCreateMicrocompactBoundaryMessage(30L, SESSION))
    ...
    void boundary_deltaPositive_yieldsBoundary() {
        MicroCompactor.setCachedMicrocompactEnabled(true);
        MicroCompactor.setPendingCacheEditsForTest(edits("auto", List.of("t1", "t2"), 100L), SESSION);

        CompactBoundaryMessage b = MicroCompactor.maybeCreateMicrocompactBoundaryMessage(250L, SESSION);
    ...
    void flowEnd_consumesPendingCacheEdits() {
        MicroCompactor.setCachedMicrocompactEnabled(true);
        // [批 3c] 会话键 = AgentState 会话：LlmAgentLoop 流结束点以 state.sessionId() 消费同一桶
        String sess = "sess-" + java.util.UUID.randomUUID().toString().substring(0, 8);
        MicroCompactor.setPendingCacheEditsForTest(edits("auto", List.of("t1"), 0L), sess);

        AgentState state = new AgentState("sys", sess, null);
        ...
        assertThat(MicroCompactor.consumePendingCacheEdits(sess))
    ...
    void flowEnd_featureOff_doesNotConsume() {
        MicroCompactor.setCachedMicrocompactEnabled(false);
        // [批 3c] 会话键 = AgentState 会话关联的显式 sessionId（与上例同构）
        String sess = "sess-" + java.util.UUID.randomUUID().toString().substring(0, 8);
        MicroCompactor.setPendingCacheEditsForTest(edits("auto", List.of("t1"), 0L), sess);
    ...
    void flowEnd_gateOn_callsMarkToolsSentToAPI() {
        MicroCompactor.setCachedMicrocompactEnabled(true);
        // [批 3c] 会话键 = AgentState 会话：cachedMCState 初始化桶必须与 LlmAgentLoop 流结束点
        //   markToolsSentToAPIState(state.sessionId()) 同一桶，否则取不到模块态、日志不可观测。
        String sess = "sess-" + java.util.UUID.randomUUID().toString().substring(0, 8);
    ...
    private static List<ChatMessageDto> buildThirteenToolMessages() {
        ...
        // [批 3c] 消息显式承载 SESSION：AnthropicSdkProvider.buildMessageParams 以
        //   SessionIdResolver.fromHistory(history) 取会话键消耗 pendingCacheEditsBlock，
        //   故 history 必须携带与 microcompactMessages 同一会话，否则两处命中不同桶。
        List<ChatMessageDto> list = new ArrayList<>();
        for (int i = 0; i < 13; i++) {
            list.add(new ChatMessageDto(
                "asst-" + i, SESSION, Role.assistant, "assistant", "thinking", null,
                List.of(new com.nexusai.model.session.dto.ToolCallDto(
                    "t" + i, "Bash", "{}", null, false)),
                FinishReason.tool_calls, null, null, "刚刚", OffsetDateTime.now(),
                null, null, null, List.of(), List.of()));
            list.add(new ChatMessageDto(
                "tool-" + i, SESSION, Role.tool, "tool", "result content", null,
    ...
    private static LlmProviderFactory completingProviderFactory() {
        ...
        Mockito.doAnswer(inv -> {
            ...
            any(), any(), (java.util.List<com.nexusai.application.agent.prompt.SystemPromptBlock>) any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any(), any(),any());
