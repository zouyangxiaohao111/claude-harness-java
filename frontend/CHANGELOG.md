...
- **测试**：新增 24 条 + 13 个反向实验；另修 **16 条**既有红测试（H1 7 / H2 6 / H3 3），逐条判定均为「测试过期」而非实现缺陷。
- **⚠️ 未修（按纪律保留，待裁定）**：2 条 `PromptShellExecutorTest` 经判定属**实现缺陷**（用前缀白名单判失败，shell 路径形状不命中 ⇒ 分支恒不可达；后果含「失败/中断命令的部分输出被当成功静默替换进提示词」） —— 该批为纯测试批，未动 `src/main`。
