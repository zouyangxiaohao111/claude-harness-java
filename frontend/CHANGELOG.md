...
### 修复
- 修复用户点「始终允许」后权限规则不生效的 bug。
- 修复会话执行过 cd / 进入工作树后，编辑记忆目录下文件反复弹权限窗的 bug。
- 修复子代理权限授权后仍弹窗的 bug。
- 修复压缩期间「停止/发送」键无法取消的 bug。

### 新增/改进
- 改进：编辑文件前的「必须先读过」校验改为会话级并在重启后恢复，减少误报。（zcw）
- 改进：编辑失败时落盘诊断信息，便于定位「找不到要替换的内容」。（zcw）

### 追加（2026-09-23 · 未递增）

- 修复对话窗口不遵守上限、随对话越用越长导致输入变卡的 bug：窗口现在固定为最近 50 轮对话，更早的会从界面卸载，点顶部「加载更早」可回看。（zcw）
- 改进：输入框打字不再重算整个消息列表，长对话下输入更跟手。（zcw）
- 修复内置浏览器插件「读取网页正文」取不到内容、或只取到一小段的 bug：现在能拿到完整正文，并可按需要缩小或聚焦读取范围。（zcw）
- ⚠️ 本次含浏览器插件修复：更新后请在浏览器「扩展程序」页**重新加载一次 NexusAI 扩展**，插件侧的改动才会生效。（zcw）
...
- **设置里的「延迟加载公告用增量方式」开关已移除**（设置 → 环境配置 → 工具加载）：这项开关现在没有作用了 —— 工具清单的公告方式已固定为「只公告有变化的部分」，不再需要你选择，因此整张「工具加载」设置卡片一并去掉。以前设过的值不再生效，你不需要做任何操作。（zcw）
- **修复：压缩后偶尔在对话里看到一大段英文原文数据**：压缩完成时注入的上下文恢复资料（文件内容、计划、技能清单、后台任务状态等）此前会当成普通消息显示在对话里，把内部数据原样露给用户。现在这些资料只给模型看，界面上不再显示；它们也不再计入消息条数统计。（zcw）
